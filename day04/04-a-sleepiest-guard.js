...
function sleepiest_guard(guards) {
  return guards.slice(0).sort((g1, g2) => {
    return g2.total_sleep - g1.total_sleep;
  })[0];
}
...
function sleepiest_minute(guard) {
  ...
  return timeline.map((v, i) => [v,i]).sort(([v1], [v2]) => v2 - v1)[0][1] * guard.id;
}

console.log(sleepiest_minute(sleepiest_guard(Object.values(guards))));
